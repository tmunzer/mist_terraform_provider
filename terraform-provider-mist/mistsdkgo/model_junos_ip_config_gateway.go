/*
Mist API

> Version: **2405.1.6** > > Date: **June 6, 2024**  ---  ### Additional Documentation * [Mist Automation Guide](https://www.juniper.net/documentation/us/en/software/mist/automation-integration/index.html) * [Mist Location SDK](https://www.juniper.net/documentation/us/en/software/mist/location-services/topics/concept/mist-how-get-mist-sdk.html) * [Mist Product Updates](https://www.mist.com/documentation/category/product-updates/)  ---  ### Helpful Resources * [API Sandbox and Exercises](https://api-class.mist.com/) * [Postman Collection, Runners and Webhook Samples](https://www.postman.com/juniper-mist/workspace/mist-systems-s-public-workspace) * [API Demo Apps](https://apps.mist-lab.fr/) * [Juniper Blog](https://blogs.juniper.net/)  --- 

API version: 2405.1.6
Contact: tmunzer@juniper.net
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mistsdkgo

import (
	"encoding/json"
)

// checks if the JunosIpConfigGateway type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &JunosIpConfigGateway{}

// JunosIpConfigGateway Junos IP Config
type JunosIpConfigGateway struct {
	// except for out-of-band interface (vme/em0/fxp0)
	Dns []string `json:"dns,omitempty"`
	// except for out-of-band interface (vme/em0/fxp0)
	DnsSuffix []string `json:"dns_suffix,omitempty"`
	// except for out-of-band interface (vme/em0/fxp0)
	Gateway *string `json:"gateway,omitempty"`
	Ip *string `json:"ip,omitempty"`
	// used only if `subnet` is not specified in `networks`
	Netmask *string `json:"netmask,omitempty"`
	// optional, the network to be used for mgmt
	Network *string `json:"network,omitempty"`
	// if `type`==`pppoe`
	PoserPassword *string `json:"poser_password,omitempty"`
	// if `type`==`pppoe`
	PpoeUsername *string `json:"ppoe_username,omitempty"`
	// if `type`==`pppoe`
	PppoeAuth *string `json:"pppoe_auth,omitempty"`
	Type *string `json:"type,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _JunosIpConfigGateway JunosIpConfigGateway

// NewJunosIpConfigGateway instantiates a new JunosIpConfigGateway object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewJunosIpConfigGateway() *JunosIpConfigGateway {
	this := JunosIpConfigGateway{}
	var pppoeAuth string = "none"
	this.PppoeAuth = &pppoeAuth
	var type_ string = "dhcp"
	this.Type = &type_
	return &this
}

// NewJunosIpConfigGatewayWithDefaults instantiates a new JunosIpConfigGateway object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewJunosIpConfigGatewayWithDefaults() *JunosIpConfigGateway {
	this := JunosIpConfigGateway{}
	var pppoeAuth string = "none"
	this.PppoeAuth = &pppoeAuth
	var type_ string = "dhcp"
	this.Type = &type_
	return &this
}

// GetDns returns the Dns field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetDns() []string {
	if o == nil || IsNil(o.Dns) {
		var ret []string
		return ret
	}
	return o.Dns
}

// GetDnsOk returns a tuple with the Dns field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetDnsOk() ([]string, bool) {
	if o == nil || IsNil(o.Dns) {
		return nil, false
	}
	return o.Dns, true
}

// HasDns returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasDns() bool {
	if o != nil && !IsNil(o.Dns) {
		return true
	}

	return false
}

// SetDns gets a reference to the given []string and assigns it to the Dns field.
func (o *JunosIpConfigGateway) SetDns(v []string) {
	o.Dns = v
}

// GetDnsSuffix returns the DnsSuffix field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetDnsSuffix() []string {
	if o == nil || IsNil(o.DnsSuffix) {
		var ret []string
		return ret
	}
	return o.DnsSuffix
}

// GetDnsSuffixOk returns a tuple with the DnsSuffix field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetDnsSuffixOk() ([]string, bool) {
	if o == nil || IsNil(o.DnsSuffix) {
		return nil, false
	}
	return o.DnsSuffix, true
}

// HasDnsSuffix returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasDnsSuffix() bool {
	if o != nil && !IsNil(o.DnsSuffix) {
		return true
	}

	return false
}

// SetDnsSuffix gets a reference to the given []string and assigns it to the DnsSuffix field.
func (o *JunosIpConfigGateway) SetDnsSuffix(v []string) {
	o.DnsSuffix = v
}

// GetGateway returns the Gateway field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetGateway() string {
	if o == nil || IsNil(o.Gateway) {
		var ret string
		return ret
	}
	return *o.Gateway
}

// GetGatewayOk returns a tuple with the Gateway field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetGatewayOk() (*string, bool) {
	if o == nil || IsNil(o.Gateway) {
		return nil, false
	}
	return o.Gateway, true
}

// HasGateway returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasGateway() bool {
	if o != nil && !IsNil(o.Gateway) {
		return true
	}

	return false
}

// SetGateway gets a reference to the given string and assigns it to the Gateway field.
func (o *JunosIpConfigGateway) SetGateway(v string) {
	o.Gateway = &v
}

// GetIp returns the Ip field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetIp() string {
	if o == nil || IsNil(o.Ip) {
		var ret string
		return ret
	}
	return *o.Ip
}

// GetIpOk returns a tuple with the Ip field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetIpOk() (*string, bool) {
	if o == nil || IsNil(o.Ip) {
		return nil, false
	}
	return o.Ip, true
}

// HasIp returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasIp() bool {
	if o != nil && !IsNil(o.Ip) {
		return true
	}

	return false
}

// SetIp gets a reference to the given string and assigns it to the Ip field.
func (o *JunosIpConfigGateway) SetIp(v string) {
	o.Ip = &v
}

// GetNetmask returns the Netmask field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetNetmask() string {
	if o == nil || IsNil(o.Netmask) {
		var ret string
		return ret
	}
	return *o.Netmask
}

// GetNetmaskOk returns a tuple with the Netmask field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetNetmaskOk() (*string, bool) {
	if o == nil || IsNil(o.Netmask) {
		return nil, false
	}
	return o.Netmask, true
}

// HasNetmask returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasNetmask() bool {
	if o != nil && !IsNil(o.Netmask) {
		return true
	}

	return false
}

// SetNetmask gets a reference to the given string and assigns it to the Netmask field.
func (o *JunosIpConfigGateway) SetNetmask(v string) {
	o.Netmask = &v
}

// GetNetwork returns the Network field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetNetwork() string {
	if o == nil || IsNil(o.Network) {
		var ret string
		return ret
	}
	return *o.Network
}

// GetNetworkOk returns a tuple with the Network field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetNetworkOk() (*string, bool) {
	if o == nil || IsNil(o.Network) {
		return nil, false
	}
	return o.Network, true
}

// HasNetwork returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasNetwork() bool {
	if o != nil && !IsNil(o.Network) {
		return true
	}

	return false
}

// SetNetwork gets a reference to the given string and assigns it to the Network field.
func (o *JunosIpConfigGateway) SetNetwork(v string) {
	o.Network = &v
}

// GetPoserPassword returns the PoserPassword field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetPoserPassword() string {
	if o == nil || IsNil(o.PoserPassword) {
		var ret string
		return ret
	}
	return *o.PoserPassword
}

// GetPoserPasswordOk returns a tuple with the PoserPassword field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetPoserPasswordOk() (*string, bool) {
	if o == nil || IsNil(o.PoserPassword) {
		return nil, false
	}
	return o.PoserPassword, true
}

// HasPoserPassword returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasPoserPassword() bool {
	if o != nil && !IsNil(o.PoserPassword) {
		return true
	}

	return false
}

// SetPoserPassword gets a reference to the given string and assigns it to the PoserPassword field.
func (o *JunosIpConfigGateway) SetPoserPassword(v string) {
	o.PoserPassword = &v
}

// GetPpoeUsername returns the PpoeUsername field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetPpoeUsername() string {
	if o == nil || IsNil(o.PpoeUsername) {
		var ret string
		return ret
	}
	return *o.PpoeUsername
}

// GetPpoeUsernameOk returns a tuple with the PpoeUsername field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetPpoeUsernameOk() (*string, bool) {
	if o == nil || IsNil(o.PpoeUsername) {
		return nil, false
	}
	return o.PpoeUsername, true
}

// HasPpoeUsername returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasPpoeUsername() bool {
	if o != nil && !IsNil(o.PpoeUsername) {
		return true
	}

	return false
}

// SetPpoeUsername gets a reference to the given string and assigns it to the PpoeUsername field.
func (o *JunosIpConfigGateway) SetPpoeUsername(v string) {
	o.PpoeUsername = &v
}

// GetPppoeAuth returns the PppoeAuth field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetPppoeAuth() string {
	if o == nil || IsNil(o.PppoeAuth) {
		var ret string
		return ret
	}
	return *o.PppoeAuth
}

// GetPppoeAuthOk returns a tuple with the PppoeAuth field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetPppoeAuthOk() (*string, bool) {
	if o == nil || IsNil(o.PppoeAuth) {
		return nil, false
	}
	return o.PppoeAuth, true
}

// HasPppoeAuth returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasPppoeAuth() bool {
	if o != nil && !IsNil(o.PppoeAuth) {
		return true
	}

	return false
}

// SetPppoeAuth gets a reference to the given string and assigns it to the PppoeAuth field.
func (o *JunosIpConfigGateway) SetPppoeAuth(v string) {
	o.PppoeAuth = &v
}

// GetType returns the Type field value if set, zero value otherwise.
func (o *JunosIpConfigGateway) GetType() string {
	if o == nil || IsNil(o.Type) {
		var ret string
		return ret
	}
	return *o.Type
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosIpConfigGateway) GetTypeOk() (*string, bool) {
	if o == nil || IsNil(o.Type) {
		return nil, false
	}
	return o.Type, true
}

// HasType returns a boolean if a field has been set.
func (o *JunosIpConfigGateway) HasType() bool {
	if o != nil && !IsNil(o.Type) {
		return true
	}

	return false
}

// SetType gets a reference to the given string and assigns it to the Type field.
func (o *JunosIpConfigGateway) SetType(v string) {
	o.Type = &v
}

func (o JunosIpConfigGateway) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o JunosIpConfigGateway) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Dns) {
		toSerialize["dns"] = o.Dns
	}
	if !IsNil(o.DnsSuffix) {
		toSerialize["dns_suffix"] = o.DnsSuffix
	}
	if !IsNil(o.Gateway) {
		toSerialize["gateway"] = o.Gateway
	}
	if !IsNil(o.Ip) {
		toSerialize["ip"] = o.Ip
	}
	if !IsNil(o.Netmask) {
		toSerialize["netmask"] = o.Netmask
	}
	if !IsNil(o.Network) {
		toSerialize["network"] = o.Network
	}
	if !IsNil(o.PoserPassword) {
		toSerialize["poser_password"] = o.PoserPassword
	}
	if !IsNil(o.PpoeUsername) {
		toSerialize["ppoe_username"] = o.PpoeUsername
	}
	if !IsNil(o.PppoeAuth) {
		toSerialize["pppoe_auth"] = o.PppoeAuth
	}
	if !IsNil(o.Type) {
		toSerialize["type"] = o.Type
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *JunosIpConfigGateway) UnmarshalJSON(data []byte) (err error) {
	varJunosIpConfigGateway := _JunosIpConfigGateway{}

	err = json.Unmarshal(data, &varJunosIpConfigGateway)

	if err != nil {
		return err
	}

	*o = JunosIpConfigGateway(varJunosIpConfigGateway)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "dns")
		delete(additionalProperties, "dns_suffix")
		delete(additionalProperties, "gateway")
		delete(additionalProperties, "ip")
		delete(additionalProperties, "netmask")
		delete(additionalProperties, "network")
		delete(additionalProperties, "poser_password")
		delete(additionalProperties, "ppoe_username")
		delete(additionalProperties, "pppoe_auth")
		delete(additionalProperties, "type")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableJunosIpConfigGateway struct {
	value *JunosIpConfigGateway
	isSet bool
}

func (v NullableJunosIpConfigGateway) Get() *JunosIpConfigGateway {
	return v.value
}

func (v *NullableJunosIpConfigGateway) Set(val *JunosIpConfigGateway) {
	v.value = val
	v.isSet = true
}

func (v NullableJunosIpConfigGateway) IsSet() bool {
	return v.isSet
}

func (v *NullableJunosIpConfigGateway) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableJunosIpConfigGateway(val *JunosIpConfigGateway) *NullableJunosIpConfigGateway {
	return &NullableJunosIpConfigGateway{value: val, isSet: true}
}

func (v NullableJunosIpConfigGateway) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableJunosIpConfigGateway) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


