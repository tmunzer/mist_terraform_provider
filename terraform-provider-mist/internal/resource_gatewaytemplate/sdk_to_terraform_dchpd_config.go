...
func dhcpdConfigVendorEncapsulatedSdkToTerraform(ctx context.Context, diags *diag.Diagnostics, d map[string]mistsdkgo.DhcpdConfigVendorOption) basetypes.MapValue {
	...
	state_value_map_attr_type := VendorEncapulatedValue{}.AttributeTypes(ctx)
	...
	for k, v := range d {
		...
		n, e := NewVendorEncapulatedValue(state_value_map_attr_type, state_value_map_attr_value)
		diags.Append(e...)
...
func dhcpdConfigOptionsSdkToTerraform(ctx context.Context, diags *diag.Diagnostics, d map[string]mistsdkgo.DhcpdConfigOption) basetypes.MapValue {
	...
	state_value_map_attr_type := OptionsValue{}.AttributeTypes(ctx)
	state_value_map_value := make(map[string]attr.Value)
	for k, v := range d {
		state_value_map_attr_value := map[string]attr.Value{
			"type":  types.StringValue(string(v.GetType())),
		...
		n, e := NewOptionsValue(state_value_map_attr_type, state_value_map_attr_value)
		diags.Append(e...)
		state_value_map_value[k] = n
	}
...
func dhcpdConfigFixedBindingsSdkToTerraform(ctx context.Context, diags *diag.Diagnostics, d map[string]mistsdkgo.DhcpdConfigFixedBinding) basetypes.MapValue {
	port_usage_type := FixedBindingsValue{}.AttributeTypes(ctx)
	...
	for k, v := range d {
		...
		port_usage_object, e := NewFixedBindingsValue(port_usage_type, port_usage_state)
		diags.Append(e...)
...
func dhcpdConfigSdkToTerraform(ctx context.Context, diags *diag.Diagnostics, d map[string]mistsdkgo.DhcpdConfig) basetypes.MapValue {
	port_usage_type := DhcpdConfigValue{}.AttributeTypes(ctx)
	state_value_map := make(map[string]attr.Value)
	for k, v := range d {
		...
		port_usage_object, e := NewDhcpdConfigValue(port_usage_type, port_usage_state)
		diags.Append(e...)
		state_value_map[k] = port_usage_object
	}
	state_type := DhcpdConfigValue{}.Type(ctx)
	state_result, e := types.MapValueFrom(ctx, state_type, state_value_map)
	diags.Append(e...)
	return state_result
}
