...
func portUsageRulesSdkToTerraform(ctx context.Context, diags *diag.Diagnostics, d []models.SwitchPortUsageDynamicRule) basetypes.ListValue {
...
func portUsagesSdkToTerraform(ctx context.Context, diags *diag.Diagnostics, d map[string]models.SwitchPortUsage) basetypes.MapValue {
	port_usage_type := PortUsagesValue{}.AttributeTypes(ctx)
	state_value_map := make(map[string]attr.Value)
	for k, v := range d {
		storm_control_state := portUsageStormControlSdkToTerraform(ctx, diags, models.SwitchPortUsageStormControl(v.GetStormControl()))
