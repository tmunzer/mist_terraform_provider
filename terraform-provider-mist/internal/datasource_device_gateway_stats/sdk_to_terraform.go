...
func SdkToTerraform(ctx context.Context, data []*models.SwitchStats) ([]DeviceSwitchStatsValue, diag.Diagnostics) {
	var state []DeviceSwitchStatsValue
	...
	for _, d := range data {
		...
		var last_trouble basetypes.ObjectValue
		var mac basetypes.StringValue
		var mac_table_stats basetypes.ObjectValue = types.ObjectNull(MacValue{}.AttributeTypes(ctx))
		...
		var module_stat basetypes.ListValue
		...
		if d.Type != nil {
			device_type = types.StringValue(*d.Type)
		}
		if d.Uptime != nil {
		...
		data_map_attr_type := DeviceSwitchStatsValue{}.AttributeTypes(ctx)
		...
		data, e := NewDeviceSwitchStatsValue(data_map_attr_type, data_map_value)
		diags.Append(e...)
