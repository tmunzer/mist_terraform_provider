...
func actTagSpecsTerraformToSdk(ctx context.Context, diags *diag.Diagnostics, d basetypes.ListValue) []mistapigo.ServiceSpec {
	var data []mistapigo.ServiceSpec
	for _, v := range d.Elements() {
		var v_interface interface{} = v
		v_state := v_interface.(SpecsValue)
		v_data := mistapigo.NewServiceSpec()
...
func TerraformToSdk(ctx context.Context, plan *ServiceModel) (mistapigo.Service, diag.Diagnostics) {
	...
	data.SetFailoverPolicy(mistapigo.ServiceFailoverPolicy(plan.FailoverPolicy.ValueString()))
	...
	data.SetTrafficClass(mistapigo.ServiceTrafficClass(plan.TrafficClass.ValueString()))
