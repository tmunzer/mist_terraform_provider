/*
Mist API

> Version: **2405.1.6** > > Date: **June 6, 2024**  ---  ### Additional Documentation * [Mist Automation Guide](https://www.juniper.net/documentation/us/en/software/mist/automation-integration/index.html) * [Mist Location SDK](https://www.juniper.net/documentation/us/en/software/mist/location-services/topics/concept/mist-how-get-mist-sdk.html) * [Mist Product Updates](https://www.mist.com/documentation/category/product-updates/)  ---  ### Helpful Resources * [API Sandbox and Exercises](https://api-class.mist.com/) * [Postman Collection, Runners and Webhook Samples](https://www.postman.com/juniper-mist/workspace/mist-systems-s-public-workspace) * [API Demo Apps](https://apps.mist-lab.fr/) * [Juniper Blog](https://blogs.juniper.net/)  --- 

API version: 2405.1.6
Contact: tmunzer@juniper.net
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mistsdkgo

import (
	"encoding/json"
)

// checks if the JunosAclTagsSpecsInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &JunosAclTagsSpecsInner{}

// JunosAclTagsSpecsInner struct for JunosAclTagsSpecsInner
type JunosAclTagsSpecsInner struct {
	// matched dst port, \"0\" means any
	PortRange *int32 `json:"port_range,omitempty"`
	// `tcp` / `udp` / `icmp` / `gre` / `any` / `:protocol_number`. `protocol_number` is between 1-254
	Protocol *string `json:"protocol,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _JunosAclTagsSpecsInner JunosAclTagsSpecsInner

// NewJunosAclTagsSpecsInner instantiates a new JunosAclTagsSpecsInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewJunosAclTagsSpecsInner() *JunosAclTagsSpecsInner {
	this := JunosAclTagsSpecsInner{}
	var portRange int32 = 0
	this.PortRange = &portRange
	var protocol string = "any"
	this.Protocol = &protocol
	return &this
}

// NewJunosAclTagsSpecsInnerWithDefaults instantiates a new JunosAclTagsSpecsInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewJunosAclTagsSpecsInnerWithDefaults() *JunosAclTagsSpecsInner {
	this := JunosAclTagsSpecsInner{}
	var portRange int32 = 0
	this.PortRange = &portRange
	var protocol string = "any"
	this.Protocol = &protocol
	return &this
}

// GetPortRange returns the PortRange field value if set, zero value otherwise.
func (o *JunosAclTagsSpecsInner) GetPortRange() int32 {
	if o == nil || IsNil(o.PortRange) {
		var ret int32
		return ret
	}
	return *o.PortRange
}

// GetPortRangeOk returns a tuple with the PortRange field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosAclTagsSpecsInner) GetPortRangeOk() (*int32, bool) {
	if o == nil || IsNil(o.PortRange) {
		return nil, false
	}
	return o.PortRange, true
}

// HasPortRange returns a boolean if a field has been set.
func (o *JunosAclTagsSpecsInner) HasPortRange() bool {
	if o != nil && !IsNil(o.PortRange) {
		return true
	}

	return false
}

// SetPortRange gets a reference to the given int32 and assigns it to the PortRange field.
func (o *JunosAclTagsSpecsInner) SetPortRange(v int32) {
	o.PortRange = &v
}

// GetProtocol returns the Protocol field value if set, zero value otherwise.
func (o *JunosAclTagsSpecsInner) GetProtocol() string {
	if o == nil || IsNil(o.Protocol) {
		var ret string
		return ret
	}
	return *o.Protocol
}

// GetProtocolOk returns a tuple with the Protocol field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JunosAclTagsSpecsInner) GetProtocolOk() (*string, bool) {
	if o == nil || IsNil(o.Protocol) {
		return nil, false
	}
	return o.Protocol, true
}

// HasProtocol returns a boolean if a field has been set.
func (o *JunosAclTagsSpecsInner) HasProtocol() bool {
	if o != nil && !IsNil(o.Protocol) {
		return true
	}

	return false
}

// SetProtocol gets a reference to the given string and assigns it to the Protocol field.
func (o *JunosAclTagsSpecsInner) SetProtocol(v string) {
	o.Protocol = &v
}

func (o JunosAclTagsSpecsInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o JunosAclTagsSpecsInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.PortRange) {
		toSerialize["port_range"] = o.PortRange
	}
	if !IsNil(o.Protocol) {
		toSerialize["protocol"] = o.Protocol
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *JunosAclTagsSpecsInner) UnmarshalJSON(data []byte) (err error) {
	varJunosAclTagsSpecsInner := _JunosAclTagsSpecsInner{}

	err = json.Unmarshal(data, &varJunosAclTagsSpecsInner)

	if err != nil {
		return err
	}

	*o = JunosAclTagsSpecsInner(varJunosAclTagsSpecsInner)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "port_range")
		delete(additionalProperties, "protocol")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableJunosAclTagsSpecsInner struct {
	value *JunosAclTagsSpecsInner
	isSet bool
}

func (v NullableJunosAclTagsSpecsInner) Get() *JunosAclTagsSpecsInner {
	return v.value
}

func (v *NullableJunosAclTagsSpecsInner) Set(val *JunosAclTagsSpecsInner) {
	v.value = val
	v.isSet = true
}

func (v NullableJunosAclTagsSpecsInner) IsSet() bool {
	return v.isSet
}

func (v *NullableJunosAclTagsSpecsInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableJunosAclTagsSpecsInner(val *JunosAclTagsSpecsInner) *NullableJunosAclTagsSpecsInner {
	return &NullableJunosAclTagsSpecsInner{value: val, isSet: true}
}

func (v NullableJunosAclTagsSpecsInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableJunosAclTagsSpecsInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


