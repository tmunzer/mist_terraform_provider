/*
Mist API

> Version: **2406.1.12** > > Date: **July 2, 2024**  ---  ### Additional Documentation * [Mist Automation Guide](https://www.juniper.net/documentation/us/en/software/mist/automation-integration/index.html) * [Mist Location SDK](https://www.juniper.net/documentation/us/en/software/mist/location_services/topics/concept/mist-how-get-mist-sdk.html) * [Mist Product Updates](https://www.mist.com/documentation/category/product-updates/)  ---  ### Helpful Resources * [API Sandbox and Exercises](https://api-class.mist.com/) * [Postman Collection, Runners and Webhook Samples](https://www.postman.com/juniper-mist/workspace/mist-systems-s-public-workspace) * [API Demo Apps](https://apps.mist-lab.fr/) * [Juniper Blog](https://blogs.juniper.net/)  --- 

API version: 2406.1.12
Contact: tmunzer@juniper.net
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mistsdkgo

import (
	"encoding/json"
)

// checks if the ResponseCountMarvisActions type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ResponseCountMarvisActions{}

// ResponseCountMarvisActions struct for ResponseCountMarvisActions
type ResponseCountMarvisActions struct {
	Distinct *string `json:"distinct,omitempty"`
	Limit *int32 `json:"limit,omitempty"`
	Results []ResponseCountMarvisActionsResult `json:"results,omitempty"`
	Total *int32 `json:"total,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _ResponseCountMarvisActions ResponseCountMarvisActions

// NewResponseCountMarvisActions instantiates a new ResponseCountMarvisActions object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewResponseCountMarvisActions() *ResponseCountMarvisActions {
	this := ResponseCountMarvisActions{}
	return &this
}

// NewResponseCountMarvisActionsWithDefaults instantiates a new ResponseCountMarvisActions object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewResponseCountMarvisActionsWithDefaults() *ResponseCountMarvisActions {
	this := ResponseCountMarvisActions{}
	return &this
}

// GetDistinct returns the Distinct field value if set, zero value otherwise.
func (o *ResponseCountMarvisActions) GetDistinct() string {
	if o == nil || IsNil(o.Distinct) {
		var ret string
		return ret
	}
	return *o.Distinct
}

// GetDistinctOk returns a tuple with the Distinct field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResponseCountMarvisActions) GetDistinctOk() (*string, bool) {
	if o == nil || IsNil(o.Distinct) {
		return nil, false
	}
	return o.Distinct, true
}

// HasDistinct returns a boolean if a field has been set.
func (o *ResponseCountMarvisActions) HasDistinct() bool {
	if o != nil && !IsNil(o.Distinct) {
		return true
	}

	return false
}

// SetDistinct gets a reference to the given string and assigns it to the Distinct field.
func (o *ResponseCountMarvisActions) SetDistinct(v string) {
	o.Distinct = &v
}

// GetLimit returns the Limit field value if set, zero value otherwise.
func (o *ResponseCountMarvisActions) GetLimit() int32 {
	if o == nil || IsNil(o.Limit) {
		var ret int32
		return ret
	}
	return *o.Limit
}

// GetLimitOk returns a tuple with the Limit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResponseCountMarvisActions) GetLimitOk() (*int32, bool) {
	if o == nil || IsNil(o.Limit) {
		return nil, false
	}
	return o.Limit, true
}

// HasLimit returns a boolean if a field has been set.
func (o *ResponseCountMarvisActions) HasLimit() bool {
	if o != nil && !IsNil(o.Limit) {
		return true
	}

	return false
}

// SetLimit gets a reference to the given int32 and assigns it to the Limit field.
func (o *ResponseCountMarvisActions) SetLimit(v int32) {
	o.Limit = &v
}

// GetResults returns the Results field value if set, zero value otherwise.
func (o *ResponseCountMarvisActions) GetResults() []ResponseCountMarvisActionsResult {
	if o == nil || IsNil(o.Results) {
		var ret []ResponseCountMarvisActionsResult
		return ret
	}
	return o.Results
}

// GetResultsOk returns a tuple with the Results field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResponseCountMarvisActions) GetResultsOk() ([]ResponseCountMarvisActionsResult, bool) {
	if o == nil || IsNil(o.Results) {
		return nil, false
	}
	return o.Results, true
}

// HasResults returns a boolean if a field has been set.
func (o *ResponseCountMarvisActions) HasResults() bool {
	if o != nil && !IsNil(o.Results) {
		return true
	}

	return false
}

// SetResults gets a reference to the given []ResponseCountMarvisActionsResult and assigns it to the Results field.
func (o *ResponseCountMarvisActions) SetResults(v []ResponseCountMarvisActionsResult) {
	o.Results = v
}

// GetTotal returns the Total field value if set, zero value otherwise.
func (o *ResponseCountMarvisActions) GetTotal() int32 {
	if o == nil || IsNil(o.Total) {
		var ret int32
		return ret
	}
	return *o.Total
}

// GetTotalOk returns a tuple with the Total field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResponseCountMarvisActions) GetTotalOk() (*int32, bool) {
	if o == nil || IsNil(o.Total) {
		return nil, false
	}
	return o.Total, true
}

// HasTotal returns a boolean if a field has been set.
func (o *ResponseCountMarvisActions) HasTotal() bool {
	if o != nil && !IsNil(o.Total) {
		return true
	}

	return false
}

// SetTotal gets a reference to the given int32 and assigns it to the Total field.
func (o *ResponseCountMarvisActions) SetTotal(v int32) {
	o.Total = &v
}

func (o ResponseCountMarvisActions) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ResponseCountMarvisActions) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Distinct) {
		toSerialize["distinct"] = o.Distinct
	}
	if !IsNil(o.Limit) {
		toSerialize["limit"] = o.Limit
	}
	if !IsNil(o.Results) {
		toSerialize["results"] = o.Results
	}
	if !IsNil(o.Total) {
		toSerialize["total"] = o.Total
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *ResponseCountMarvisActions) UnmarshalJSON(data []byte) (err error) {
	varResponseCountMarvisActions := _ResponseCountMarvisActions{}

	err = json.Unmarshal(data, &varResponseCountMarvisActions)

	if err != nil {
		return err
	}

	*o = ResponseCountMarvisActions(varResponseCountMarvisActions)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "distinct")
		delete(additionalProperties, "limit")
		delete(additionalProperties, "results")
		delete(additionalProperties, "total")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableResponseCountMarvisActions struct {
	value *ResponseCountMarvisActions
	isSet bool
}

func (v NullableResponseCountMarvisActions) Get() *ResponseCountMarvisActions {
	return v.value
}

func (v *NullableResponseCountMarvisActions) Set(val *ResponseCountMarvisActions) {
	v.value = val
	v.isSet = true
}

func (v NullableResponseCountMarvisActions) IsSet() bool {
	return v.isSet
}

func (v *NullableResponseCountMarvisActions) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableResponseCountMarvisActions(val *ResponseCountMarvisActions) *NullableResponseCountMarvisActions {
	return &NullableResponseCountMarvisActions{value: val, isSet: true}
}

func (v NullableResponseCountMarvisActions) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableResponseCountMarvisActions) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


