/*
Mist API

> Version: **2406.1.11** > > Date: **July 1, 2024**  ---  ### Additional Documentation * [Mist Automation Guide](https://www.juniper.net/documentation/us/en/software/mist/automation-integration/index.html) * [Mist Location SDK](https://www.juniper.net/documentation/us/en/software/mist/location_services/topics/concept/mist-how-get-mist-sdk.html) * [Mist Product Updates](https://www.mist.com/documentation/category/product-updates/)  ---  ### Helpful Resources * [API Sandbox and Exercises](https://api-class.mist.com/) * [Postman Collection, Runners and Webhook Samples](https://www.postman.com/juniper-mist/workspace/mist-systems-s-public-workspace) * [API Demo Apps](https://apps.mist-lab.fr/) * [Juniper Blog](https://blogs.juniper.net/)  --- 

API version: 2406.1.11
Contact: tmunzer@juniper.net
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mistsdkgo

import (
	"encoding/json"
	"fmt"
)

// checks if the ZoneStatsDetailsClientWaits type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ZoneStatsDetailsClientWaits{}

// ZoneStatsDetailsClientWaits client wait time right now
type ZoneStatsDetailsClientWaits struct {
	// average wait time in seconds
	Avg int32 `json:"avg"`
	// longest wait time in seconds
	Max int32 `json:"max"`
	// shortest wait time in seconds
	Min int32 `json:"min"`
	// 95th percentile of all the wait time(s)
	P95 int32 `json:"p95"`
	AdditionalProperties map[string]interface{}
}

type _ZoneStatsDetailsClientWaits ZoneStatsDetailsClientWaits

// NewZoneStatsDetailsClientWaits instantiates a new ZoneStatsDetailsClientWaits object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewZoneStatsDetailsClientWaits(avg int32, max int32, min int32, p95 int32) *ZoneStatsDetailsClientWaits {
	this := ZoneStatsDetailsClientWaits{}
	this.Avg = avg
	this.Max = max
	this.Min = min
	this.P95 = p95
	return &this
}

// NewZoneStatsDetailsClientWaitsWithDefaults instantiates a new ZoneStatsDetailsClientWaits object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewZoneStatsDetailsClientWaitsWithDefaults() *ZoneStatsDetailsClientWaits {
	this := ZoneStatsDetailsClientWaits{}
	return &this
}

// GetAvg returns the Avg field value
func (o *ZoneStatsDetailsClientWaits) GetAvg() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Avg
}

// GetAvgOk returns a tuple with the Avg field value
// and a boolean to check if the value has been set.
func (o *ZoneStatsDetailsClientWaits) GetAvgOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Avg, true
}

// SetAvg sets field value
func (o *ZoneStatsDetailsClientWaits) SetAvg(v int32) {
	o.Avg = v
}

// GetMax returns the Max field value
func (o *ZoneStatsDetailsClientWaits) GetMax() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Max
}

// GetMaxOk returns a tuple with the Max field value
// and a boolean to check if the value has been set.
func (o *ZoneStatsDetailsClientWaits) GetMaxOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Max, true
}

// SetMax sets field value
func (o *ZoneStatsDetailsClientWaits) SetMax(v int32) {
	o.Max = v
}

// GetMin returns the Min field value
func (o *ZoneStatsDetailsClientWaits) GetMin() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Min
}

// GetMinOk returns a tuple with the Min field value
// and a boolean to check if the value has been set.
func (o *ZoneStatsDetailsClientWaits) GetMinOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Min, true
}

// SetMin sets field value
func (o *ZoneStatsDetailsClientWaits) SetMin(v int32) {
	o.Min = v
}

// GetP95 returns the P95 field value
func (o *ZoneStatsDetailsClientWaits) GetP95() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.P95
}

// GetP95Ok returns a tuple with the P95 field value
// and a boolean to check if the value has been set.
func (o *ZoneStatsDetailsClientWaits) GetP95Ok() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.P95, true
}

// SetP95 sets field value
func (o *ZoneStatsDetailsClientWaits) SetP95(v int32) {
	o.P95 = v
}

func (o ZoneStatsDetailsClientWaits) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ZoneStatsDetailsClientWaits) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["avg"] = o.Avg
	toSerialize["max"] = o.Max
	toSerialize["min"] = o.Min
	toSerialize["p95"] = o.P95

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *ZoneStatsDetailsClientWaits) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"avg",
		"max",
		"min",
		"p95",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varZoneStatsDetailsClientWaits := _ZoneStatsDetailsClientWaits{}

	err = json.Unmarshal(data, &varZoneStatsDetailsClientWaits)

	if err != nil {
		return err
	}

	*o = ZoneStatsDetailsClientWaits(varZoneStatsDetailsClientWaits)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "avg")
		delete(additionalProperties, "max")
		delete(additionalProperties, "min")
		delete(additionalProperties, "p95")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableZoneStatsDetailsClientWaits struct {
	value *ZoneStatsDetailsClientWaits
	isSet bool
}

func (v NullableZoneStatsDetailsClientWaits) Get() *ZoneStatsDetailsClientWaits {
	return v.value
}

func (v *NullableZoneStatsDetailsClientWaits) Set(val *ZoneStatsDetailsClientWaits) {
	v.value = val
	v.isSet = true
}

func (v NullableZoneStatsDetailsClientWaits) IsSet() bool {
	return v.isSet
}

func (v *NullableZoneStatsDetailsClientWaits) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableZoneStatsDetailsClientWaits(val *ZoneStatsDetailsClientWaits) *NullableZoneStatsDetailsClientWaits {
	return &NullableZoneStatsDetailsClientWaits{value: val, isSet: true}
}

func (v NullableZoneStatsDetailsClientWaits) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableZoneStatsDetailsClientWaits) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


